...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut user_copy = user.clone();
    and_path.apply_operation(&mut user_copy, "replace", &json!("primary-work@company.com"))?;
    
    let emails = user_copy["emails"].as_array().unwrap();
    let primary_work: Vec<_> = emails.iter()
        .filter(|e| e["type"] == "work" && e["primary"] == true)
        .collect();
    println!("   Result: Updated {} primary work email(s)", primary_work.len());
    ...
    println!("   Result: Removed {} phone number(s)", original_count - remaining_count);
    ...
    println!("   Result: Removed {} email(s)", original_count - remaining_count);
    ...
    let mut user_copy = user.clone();
    advanced_path.apply_operation(&mut user_copy, "replace", &json!({
    ...
    let company_emails: Vec<_> = emails.iter()
        ...
        .collect();
    println!("   Result: Updated {} company email(s)", company_emails.len());
