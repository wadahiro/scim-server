...
use crate::models::{ScimPatchOp, User, ScimListResponse};
...
fn fix_user_refs(base_url: &str, tenant_info: &TenantInfo, user: &mut User) {
...
pub async fn create_user(
    ...
    match backend.create_user(tenant_id, &user).await {
        Ok(mut created_user) => {
            ...
            fix_user_refs(&base_url, &tenant_info, &mut created_user);
...
pub async fn get_user(
    ...
    match backend.find_user_by_id(tenant_id, &id).await {
        Ok(Some(mut user)) => {
            fix_user_refs(&base_url, &tenant_info, &mut user);
...
pub async fn search_users(
    ...
    if let Some(filter_str) = filter {
        if filter_str.starts_with("groups[value eq ") && filter_str.ends_with("]") {
            ...
            match backend.find_users_by_group_id(tenant_id, group_id).await {
                Ok(mut users) => {
                    // Fix refs for all users
                    for user in &mut users {
                        fix_user_refs(&base_url, &tenant_info, user);
                    }
    ...
    if let Some(filter_str) = filter {
        match parse_filter(filter_str) {
            Ok(filter_op) => {
                ...
                match backend.find_users_by_filter(tenant_id, &filter_op, start_index, count, sort_spec.as_ref()).await {
                    Ok((mut users, total)) => {
                        // Fix refs for all users
                        for user in &mut users {
                            fix_user_refs(&base_url, &tenant_info, user);
    ...
    let result = if sort_spec.is_some() {
        backend.find_all_users_sorted(tenant_id, start_index, count, sort_spec.as_ref()).await
    } else {
...
pub async fn update_user(
    ...
    match backend.update_user(tenant_id, &id, &user).await {
        Ok(Some(mut updated_user)) => {
            fix_user_refs(&base_url, &tenant_info, &mut updated_user);
