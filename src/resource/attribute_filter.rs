...
use crate::schema::definitions::{Returned, find_attribute, USER_SCHEMA, GROUP_SCHEMA};
...
impl AttributeFilter {
    ...
    pub fn apply_to_resource(
        ...
        // If no filtering specified, return original resource
        ...
        // Filter the resource
        self.filter_json_object(resource, &included_attributes)
    }
        ...
        for attr in &schema.attributes {
            match attr.returned {
                Returned::Always | Returned::Default => {
                    ...
                    self.add_sub_attributes_recursive(&attr.name, &attr.sub_attributes, &mut included);
                }
    ...
    fn should_include_attribute(
        ...
        included_attributes.iter().any(|included| {
            included.starts_with(&format!("{}.", attr_name))
        })
    }
