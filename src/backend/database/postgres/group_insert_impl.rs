...
impl PostgresGroupInserter {
    ...
    async fn check_duplicate_display_name(&self, tenant_id: u32, display_name: &str) -> AppResult<()> {
        let table_name = format!("t{}_groups", tenant_id);
        let sql = format!("SELECT COUNT(*) FROM {} WHERE LOWER(display_name) = LOWER($1)", table_name);
...
impl GroupInserter for PostgresGroupInserter {
    async fn execute_group_insert(
        ...
        self.check_duplicate_display_name(tenant_id, &data.display_name).await?;
            ...
            for member in members {
                if let Some(member_id) = &member.value {
                    ...
                    sqlx::query(&membership_sql)
                        ...
                        .execute(&mut *tx)
                        .await
                        .map_err(|e| crate::error::AppError::Database(format!("Failed to insert group member: {}", e)))?;
                }
            }
        }
        ...
        // Commit transaction
        tx.commit().await
            .map_err(|e| crate::error::AppError::Database(format!("Failed to commit transaction: {}", e)))?;
