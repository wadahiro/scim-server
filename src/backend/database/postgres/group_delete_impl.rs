...
impl GroupDeleter for PostgresGroupDeleter {
    async fn execute_group_delete(&self, tenant_id: u32, id: &str) -> AppResult<bool> {
        ...
        let group_sql = format!("DELETE FROM {} WHERE id = $1::uuid", group_table);
...
mod tests {
    ...
    fn test_validate_uuid_format() {
        // Valid UUIDs
        assert!(
            PostgresGroupDeleter::validate_uuid_format("123e4567-e89b-12d3-a456-426614174000")
                .is_ok()
        );
        assert!(
            PostgresGroupDeleter::validate_uuid_format("00000000-0000-0000-0000-000000000000")
                .is_ok()
        );
