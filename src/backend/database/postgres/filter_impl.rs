...
impl PostgresFilterConverter {
    ...
    fn handle_equality(
        ...
        if let Value::Bool(bool_val) = value {
            ...
            return Ok(format!("data_norm #> '{{{}}}' = '{}'", json_path, if *bool_val { "true" } else { "false" }));
        }
        ...
        Ok(format!("{} #>> '{{{}}}' = ${}", data_column, json_path, param_index))
    }
    ...
    fn handle_contains(
        ...
        Ok(format!("LOWER(data_norm #>> '{{{}}}') LIKE LOWER(${})", json_path, param_index))
    }
    ...
    fn handle_present(
        ...
        let json_path = self.scim_path_to_json_path(attr, resource_type);
        Ok(format!("data_norm #>> '{{{}}}' IS NOT NULL AND data_norm #>> '{{{}}}' != ''", json_path, json_path))
    }
    ...
    fn handle_greater_than(
        ...
        Ok(format!("(data_norm #>> '{{{}}}')::numeric > ${}::numeric", json_path, param_index))
    }
...
mod tests {
    ...
    fn test_filter_conversion() {
        let converter = PostgresFilterConverter::new();
        let filter = FilterOperator::Equal("userName".to_string(), serde_json::Value::String("john.doe".to_string()));
    ...
    fn test_not_filter_conversion() {
        let converter = PostgresFilterConverter::new();
        let inner_filter = FilterOperator::Equal("active".to_string(), serde_json::Value::Bool(true));
    ...
    fn test_complex_filter_conversion() {
        let converter = PostgresFilterConverter::new();
        let inner_filter = FilterOperator::Equal("type".to_string(), serde_json::Value::String("work".to_string()));
    ...
    fn test_not_with_complex_filter_conversion() {
        let converter = PostgresFilterConverter::new();
        let inner_filter = FilterOperator::Equal("value".to_string(), serde_json::Value::String("alice@example.com".to_string()));
        let complex_filter = FilterOperator::Complex("emails".to_string(), Box::new(inner_filter));
