...
/// PostgreSQL-specific implementation of UserReader
...
impl PostgresUserReader {
        ...
        match row {
            Some(row) => {
                ...
                *user.password_mut() = None;
                ...
                let existing_location = user.meta().as_ref().and_then(|m| m.location.clone());
    ...
    async fn fetch_user_groups(&self, tenant_id: u32, user_id: &str) -> AppResult<Vec<UserGroup>> {
        ...
        let sql = format!(
            ...
            SELECT 
                g.id,
                g.data_orig->>'displayName' as display_name
            FROM {} g
        ...
        for row in rows {
            ...
            let ref_url = format!("/{}/Groups/{}", tenant_id, group_id_string);

            groups.push(UserGroup {
                value: Some(group_id_string),
                ref_: Some(ref_url),
    ...
    fn filter_converter(
        &self,
    ) -> crate::backend::database::postgres::filter_impl::PostgresFilterConverter {
...
impl UserReader for PostgresUserReader {
    ...
    async fn find_user_by_username(
        ...
        match row {
            Some(row) => {
                ...
                self.fetch_user_with_groups_optional(tenant_id, &id_string, include_groups)
                    .await
            }
            None => Ok(None),
        }
    ...
    async fn find_all_users(
        ...
        for row in rows {
            ...
            if let Some(user) = self
                .fetch_user_with_groups_optional(tenant_id, &id_string, include_groups)
                .await?
            {
                users.push(user);
            }
    ...
    async fn find_users_by_group_id(
        ...
        let sql = format!(
            ...
            WHERE m.group_id = $1::uuid AND m.member_type = 'User'
