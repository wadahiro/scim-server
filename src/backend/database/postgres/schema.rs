...
async fn create_indexes(pool: &PgPool, tenant_id: u32) -> AppResult<()> {
    ...
    let membership_indexes = vec![
        format!("CREATE INDEX IF NOT EXISTS \"idx_{}_memberships_group_id\" ON {} (group_id)", tenant_id, memberships_table),
        format!("CREATE INDEX IF NOT EXISTS \"idx_{}_memberships_member_id\" ON {} (member_id)", tenant_id, memberships_table),
        format!("CREATE INDEX IF NOT EXISTS \"idx_{}_memberships_member_type\" ON {} (member_type)", tenant_id, memberships_table),
    ];
