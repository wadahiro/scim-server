...
use super::super::user_patch::{UserPatcher, PreparedUserPatchData};
...
impl PostgresUserPatcher {
    ...
    async fn check_duplicate_username(&self, tenant_id: u32, username: &str, exclude_id: &str) -> AppResult<()> {
        ...
        let sql = format!("SELECT COUNT(*) FROM {} WHERE LOWER(username) = LOWER($1) AND id != $2::uuid", table_name);
...
pub fn map_database_error(e: sqlx::Error, resource_type: &str) -> AppError {
    match e {
        ...
        _ => AppError::Database(format!("Failed to patch {}: {}", resource_type.to_lowercase(), e)),
    }
}
