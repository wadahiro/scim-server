    ...
    pub fn prepare_group_for_update(id: &str, group: &Group) -> AppResult<PreparedGroupUpdateData> {
        ...
        *group.id_mut() = id.to_string();
        ...
        let members = group.members().as_ref().map(|members| {
            members.iter().map(|member| scim_v2::models::group::Member {
        ...
        *group_for_storage.members_mut() = None;
        ...
        let data_orig = serde_json::to_value(&group_for_storage)
...
impl<T: GroupUpdater> UnifiedGroupUpdateOps<T> {
    ...
    pub async fn update_group(&self, tenant_id: u32, id: &str, group: &Group) -> AppResult<Option<Group>> {
        // Validate inputs
        ...
        self.updater.execute_group_update(tenant_id, id, prepared).await
    }
}
...
mod tests {
    ...
    fn test_validate_group_id() {
        ...
        assert!(GroupUpdateProcessor::validate_group_id("123e4567-e89b-12d3-a456-426614174000").is_ok());
        ...
        if let Some(base_obj) = data_obj.get("base") {
            if let Some(base_obj) = base_obj.as_object() {
                ...
                if let Some(members_value) = base_obj.get("members") {
                    assert!(members_value.is_null(), "Members should be null in JSON data, but was: {:?}", members_value);
                }
