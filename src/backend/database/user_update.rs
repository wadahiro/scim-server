...
impl<T: UserUpdater> UnifiedUserUpdateOps<T> {
    ...
    pub async fn update_user(&self, tenant_id: u32, id: &str, user: &User) -> AppResult<Option<User>> {
        // Validate inputs
        ...
        let result = self.updater.execute_user_update(tenant_id, id, prepared).await?;
