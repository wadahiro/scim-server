...
impl<T: UserReader> UnifiedUserReadOps<T> {
    ...
    /// Find a user by username (case-insensitive)
    pub async fn find_user_by_username(&self, tenant_id: u32, username: &str) -> AppResult<Option<User>> {
    ...
    pub async fn find_all_users(
        ...
        self.reader.find_all_users(tenant_id, start_index, count).await
    }
    ...
    pub async fn find_all_users_sorted(
        ...
        self.reader.find_all_users_sorted(tenant_id, start_index, count, sort_spec).await
    }
    ...
    pub async fn find_users_by_filter(
        ...
        self.reader.find_users_by_filter(tenant_id, filter, start_index, count, sort_spec).await
    }
    
    /// Find users by group ID
    pub async fn find_users_by_group_id(&self, tenant_id: u32, group_id: &str) -> AppResult<Vec<User>> {
        self.reader.find_users_by_group_id(tenant_id, group_id).await
    }
}