...
async fn create_indexes(pool: &SqlitePool, tenant_id: u32) -> AppResult<()> {
    ...
    let user_indexes = vec![
        format!("CREATE INDEX IF NOT EXISTS idx_{}_users_username ON {} (LOWER(username))", sanitized_tenant_id, users_table),
        format!("CREATE INDEX IF NOT EXISTS idx_{}_users_external_id ON {} (external_id)", sanitized_tenant_id, users_table),
        format!("CREATE INDEX IF NOT EXISTS idx_{}_users_created_at ON {} (created_at)", sanitized_tenant_id, users_table),
    ];
    
    // Groups table indexes
    let group_indexes = vec![
        format!("CREATE INDEX IF NOT EXISTS idx_{}_groups_display_name ON {} (LOWER(display_name))", sanitized_tenant_id, groups_table),
        format!("CREATE INDEX IF NOT EXISTS idx_{}_groups_external_id ON {} (external_id)", sanitized_tenant_id, groups_table),
        format!("CREATE INDEX IF NOT EXISTS idx_{}_groups_created_at ON {} (created_at)", sanitized_tenant_id, groups_table),
    ];
    
    // Memberships table indexes
    let membership_indexes = vec![
        format!("CREATE INDEX IF NOT EXISTS idx_{}_memberships_group_id ON {} (group_id)", sanitized_tenant_id, memberships_table),
        format!("CREATE INDEX IF NOT EXISTS idx_{}_memberships_member_id ON {} (member_id)", sanitized_tenant_id, memberships_table),
        format!("CREATE INDEX IF NOT EXISTS idx_{}_memberships_member_type ON {} (member_type)", sanitized_tenant_id, memberships_table),
    ];
    
    // Execute all index creation queries
    for sql in user_indexes.iter().chain(group_indexes.iter()).chain(membership_indexes.iter()) {
        sqlx::query(sql)
            .execute(pool)
            .await
