...
/// SQLite-specific filter converter for SCIM filters
/// 
/// This handles conversion of SCIM filter expressions to SQLite
...
impl FilterConverter for SqliteFilterConverter {
    fn to_where_clause(
        &self,
    ...
    fn get_param_placeholder(&self, _index: usize) -> String {
    ...
    fn get_json_path(&self, attr: &str, resource_type: ResourceType) -> String {
        // SQLite uses json_extract() function for JSON access
        match resource_type {
            ResourceType::User => match attr {
                ...
                _ if attr.starts_with("meta.") => {
                    let sub_attr = &attr[5..];
                    format!("json_extract(data_norm, '$.meta.{}')", sub_attr)
                }
                _ => format!("json_extract(data_norm, '$.{}')", attr),
            },
...
impl SqliteFilterConverter {
    ...
    fn convert_filter_to_sql(
        ...
        match filter {
            ...
            FilterOperator::Present(attr) => {
                self.handle_present(attr, resource_type)
            }
    ...
    fn handle_equality(
        ...
        if let Value::Bool(bool_val) = value {
            ...
            return Ok(format!("json_extract(data_norm, '$.{}') = {}", json_path, if *bool_val { "1" } else { "0" }));
        }
        
        // Check if this is a case-exact field
        let is_case_exact = self.is_case_exact_field(attr, resource_type);
        let data_column = if is_case_exact { "data_orig" } else { "data_norm" };
        ...
        Ok(format!("json_extract({}, '$.{}') = ?{}", data_column, json_path, param_index))
    }
    ...
    fn handle_contains(
        ...
        Ok(format!("LOWER(json_extract(data_norm, '$.{}')) LIKE LOWER(?{})", json_path, param_index))
    }
    ...
    fn handle_present(
        ...
        let json_path = self.scim_path_to_json_path(attr, resource_type);
        Ok(format!("json_extract(data_norm, '$.{}') IS NOT NULL AND json_extract(data_norm, '$.{}') != ''", json_path, json_path))
    }
    ...
    fn handle_greater_than(
        ...
        Ok(format!("CAST(json_extract(data_norm, '$.{}') AS REAL) > CAST(?{} AS REAL)", json_path, param_index))
    }
    ...
    /// Convert SCIM attribute path to SQLite JSON path
    ...
    fn handle_complex_filter(
        ...
        match inner {
            ...
            FilterOperator::Present(sub_attr) => {
                ...
                Ok(format!(
                    "EXISTS (SELECT 1 FROM json_each(data_norm, '$.{}') WHERE json_extract(value, '$.{}') IS NOT NULL)",
                    attr.to_lowercase(),
            ...
            _ => {
                Err(crate::error::AppError::FilterParse(
                    format!("Unsupported complex filter operation for {}", attr)
                ))
            }
        }
    }
    ...
    fn handle_multi_value_equality(
        ...
        // Use SQLite JSON functions to search in array
        // This creates a query like: EXISTS (SELECT 1 FROM json_each(data_norm, '$.emails') WHERE json_extract(value, '$.value') = ?)
        Ok(format!(
            "EXISTS (SELECT 1 FROM json_each(data_norm, '$.{}') WHERE json_extract(value, '$.{}') = ?{})",
    ...
    fn handle_multi_value_not_equality(
        ...
        Ok(format!(
            "NOT EXISTS (SELECT 1 FROM json_each(data_norm, '$.{}') WHERE json_extract(value, '$.{}') = ?{})",
            attr_name.to_lowercase(),
            sub_attr.to_lowercase(),
            param_index
        ))
    ...
    fn handle_multi_value_contains(
        ...
        // Use SQLite JSON functions to search in array with LIKE
        Ok(format!(
            "EXISTS (SELECT 1 FROM json_each(data_norm, '$.{}') WHERE LOWER(json_extract(value, '$.{}')) LIKE LOWER(?{}))",
            attr_name.to_lowercase(),
            sub_attr.to_lowercase(),
            param_index
        ))
...
mod tests {
    ...
    fn test_scim_path_conversion() {
        let converter = SqliteFilterConverter::new();
    ...
    fn test_filter_conversion() {
        let converter = SqliteFilterConverter::new();
        let filter = FilterOperator::Equal("userName".to_string(), Value::String("john.doe".to_string()));
        
        let (condition, params) = converter
            .to_where_clause(&filter, ResourceType::User)
            .unwrap();
        
        assert_eq!(condition, "json_extract(data_norm, '$.username') = ?1");
    ...
    fn test_not_filter_conversion() {
        let converter = SqliteFilterConverter::new();
        let inner_filter = FilterOperator::Equal("active".to_string(), Value::Bool(true));
        ...
        // Boolean values use direct comparison without parameter binding (1 for true, 0 for false in SQLite)
        assert_eq!(condition, "NOT (json_extract(data_norm, '$.active') = 1)");
    ...
    fn test_complex_filter_conversion() {
        let converter = SqliteFilterConverter::new();
        let inner_filter = FilterOperator::Equal("type".to_string(), Value::String("work".to_string()));
        ...
        assert_eq!(condition, "EXISTS (SELECT 1 FROM json_each(data_norm, '$.emails') WHERE json_extract(value, '$.type') = ?1)");
    ...
    fn test_not_with_complex_filter_conversion() {
        let converter = SqliteFilterConverter::new();
        let inner_filter = FilterOperator::Equal("value".to_string(), Value::String("alice@example.com".to_string()));
        let complex_filter = FilterOperator::Complex("emails".to_string(), Box::new(inner_filter));
        ...
        assert_eq!(condition, "NOT (EXISTS (SELECT 1 FROM json_each(data_norm, '$.emails') WHERE json_extract(value, '$.value') = ?1))");
