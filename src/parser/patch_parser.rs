...
fn is_multi_valued_attribute(attr_name: &str) -> bool {
    matches!(attr_name, 
        "emails" | "phoneNumbers" | "addresses" | "photos" | 
        "ims" | "entitlements" | "roles" | "x509Certificates"
    )
}
...
impl ScimFilter {
    ...
    fn extract_first_condition<'a>(&self, filter_op: &'a FilterOperator) -> (&'a str, &'a str, &'a str) {
        match filter_op {
            ...
            FilterOperator::GreaterThanOrEqual(attr, val) => (attr, "ge", &val.as_str().unwrap_or("")),
    ...
    fn evaluate_filter(&self, item_obj: &serde_json::Map<String, Value>, filter_op: &FilterOperator) -> bool {
        match filter_op {
            ...
            FilterOperator::GreaterThanOrEqual(attr, val) => self.simple_match(item_obj, attr, "ge", val),
            FilterOperator::LessThan(attr, val) => self.simple_match(item_obj, attr, "lt", val),
            FilterOperator::LessThanOrEqual(attr, val) => self.simple_match(item_obj, attr, "le", val),
    ...
    fn simple_match(&self, item_obj: &serde_json::Map<String, Value>, attribute: &str, operator: &str, expected_value: &Value) -> bool {
        ...
        match operator {
            ...
            "gt" => self.compare_values(actual_value, expected_value) == std::cmp::Ordering::Greater,
            "ge" => matches!(self.compare_values(actual_value, expected_value), std::cmp::Ordering::Greater | std::cmp::Ordering::Equal),
            "lt" => self.compare_values(actual_value, expected_value) == std::cmp::Ordering::Less,
            "le" => matches!(self.compare_values(actual_value, expected_value), std::cmp::Ordering::Less | std::cmp::Ordering::Equal),
            _ => false,
        }
    }
