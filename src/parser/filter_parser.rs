...
pub fn parse_filter(filter_str: &str) -> AppResult<FilterOperator> {
    ...
    if let Some(bracket_pos) = trimmed.find('[') {
        if let Some(bracket_end) = trimmed.rfind(']') {
            ...
            eprintln!(
                "DEBUG parser: complex filter - attr='{}', filter_expr='{}'",
                attr, filter_expr
            );
            ...
            return Ok(FilterOperator::Complex(
                attr.to_string(),
    ...
    for (op_str, op_type) in &operators {
        if let Some(op_pos) = find_operator_position(trimmed, op_str) {
            ...
            return match *op_type {
                ...
                "GreaterThanOrEqual" => {
                    Ok(FilterOperator::GreaterThanOrEqual(attr.to_string(), value))
                }
                ...
                _ => Err(AppError::FilterParse(format!(
                    "Unknown operator: {}",
                    op_type
                ))),
            };
                ...
                if !in_quotes && i + operator.len() <= text.len() {
                    ...
                    if &text_bytes[i..i + operator.len()] == op_bytes {
                        ...
                        if operator.chars().all(|c| c.is_alphabetic()) {
                            ...
                            let after_ok = i + operator.len() >= text.len()
                                || !text_bytes[i + operator.len()].is_ascii_alphabetic();
...
mod tests {
    ...
    fn test_simple_equal_filter() {
        ...
        assert_eq!(
            result,
            FilterOperator::Equal("userName".to_string(), json!("john.doe"))
        );
    }
    ...
    fn test_equal_with_spaces() {
        ...
        assert_eq!(
            result,
            FilterOperator::Equal("title".to_string(), json!("Product Manager"))
        );
    }
    ...
    fn test_complex_filter_with_brackets() {
        ...
        assert_eq!(
            result,
            FilterOperator::Complex(
                "emails".to_string(),
                Box::new(FilterOperator::Equal(
                    "value".to_string(),
    ...
    fn test_contains_operator() {
        ...
        assert_eq!(
            result,
            FilterOperator::Contains("displayName".to_string(), json!("John"))
        );
    }
    ...
    fn test_numeric_filter() {
        ...
        assert_eq!(
            result,
            FilterOperator::GreaterThan("age".to_string(), json!(30))
        );
    }
    ...
    fn test_dot_notation() {
        ...
        assert_eq!(
            result,
            FilterOperator::Equal("name.givenName".to_string(), json!("John"))
        );
    }
    ...
    fn test_and_operator() {
        ...
        match result {
            FilterOperator::And(left, right) => {
                assert_eq!(
                    *left,
                    FilterOperator::Equal("userName".to_string(), json!("john"))
                );
                assert_eq!(
                    *right,
                    FilterOperator::Equal("active".to_string(), json!(true))
                );
            }
    ...
    fn test_or_operator() {
        ...
        match result {
            FilterOperator::Or(left, right) => {
                ...
                assert_eq!(
                    *right,
                    FilterOperator::Equal("userName".to_string(), json!("jane"))
                );
            }
            ...
            FilterOperator::Or(left, right) => {
                ...
                match *right {
                    FilterOperator::And(and_left, and_right) => {
                        assert_eq!(
                            *and_left,
                            FilterOperator::Equal("b".to_string(), json!("2"))
                        );
                        assert_eq!(
                            *and_right,
                            FilterOperator::Equal("c".to_string(), json!("3"))
                        );
                    }
    ...
    fn test_complex_parentheses_precedence() {
        ...
        let result =
            parse_filter("(userName eq \"admin\" or userName eq \"manager\") and active eq true")
                .unwrap();
        match result {
            FilterOperator::And(left, right) => {
                match *left {
                    FilterOperator::Or(or_left, or_right) => {
                        assert_eq!(
                            *or_left,
                            FilterOperator::Equal("userName".to_string(), json!("admin"))
                        );
                        assert_eq!(
                            *or_right,
                            FilterOperator::Equal("userName".to_string(), json!("manager"))
                        );
                    }
            ...
            FilterOperator::Or(left, right) => {
                ...
                match *left {
                    FilterOperator::And(and_left, and_right) => {
                        ...
                        assert_eq!(
                            *and_right,
                            FilterOperator::Equal("role".to_string(), json!("admin"))
                        );
                    }
                ...
                assert_eq!(
                    *right,
                    FilterOperator::Equal("active".to_string(), json!(false))
                );
            }
            _ => panic!("Expected Or operator at top level"),
        }
    ...
    fn test_complex_filter_with_type() {
        ...
        assert_eq!(
            ...
            FilterOperator::Complex(
                "emails".to_string(),
                Box::new(FilterOperator::Equal("type".to_string(), json!("work")))
            )
        );
    }
    ...
    fn test_complex_filter_with_contains() {
        ...
        assert_eq!(
            ...
            FilterOperator::Complex(
                ...
                Box::new(FilterOperator::Contains("value".to_string(), json!("555")))
            )
        );
    }
    ...
    fn test_complex_filter_with_present() {
        ...
        assert_eq!(
            ...
            FilterOperator::Complex(
                "addresses".to_string(),
                Box::new(FilterOperator::Present("type".to_string()))
            )
        );
    }
    ...
    fn test_not_operator_simple() {
        ...
        assert_eq!(
            result,
            FilterOperator::Not(Box::new(FilterOperator::Equal(
                "active".to_string(),
                json!(true)
            )))
        );
    ...
    fn test_not_operator_with_parentheses() {
        ...
        match result {
            FilterOperator::Not(inner) => match *inner {
                FilterOperator::And(left, right) => {
                    assert_eq!(
                        *left,
                        FilterOperator::Equal("userName".to_string(), json!("john"))
    ...
    fn test_not_with_complex_filter() {
        ...
        assert_eq!(
            result,
            FilterOperator::Not(Box::new(FilterOperator::Complex(
                "emails".to_string(),
                Box::new(FilterOperator::Equal("type".to_string(), json!("work")))
            )))
        );
