...
impl TenantConfig {
    ...
    fn resolve_url_from_request(&self, request_info: &RequestInfo) -> Option<ResolvedUrl> {
        ...
        match host_resolution.resolution_type {
            HostResolutionType::Host => {
                self.resolve_from_host_header(request_info)
            }
            HostResolutionType::Forwarded => {
                self.resolve_from_forwarded_header(request_info)
            }
            HostResolutionType::XForwarded => {
                self.resolve_from_x_forwarded_headers(request_info)
            }
        }
    }
    ...
    fn resolve_from_x_forwarded_headers(&self, request_info: &RequestInfo) -> Option<ResolvedUrl> {
        let scheme = request_info.x_forwarded_proto
            .unwrap_or("https")
            .to_string();
...
impl AppConfig {
            ...
            if i + 1 < chars.len() && chars[i] == '$' && chars[i + 1] == '{' {
                ...
                if j < chars.len() {
                    ...
                    let (var_name, default_value) = if let Some(pos) = var_expr.find(":-") {
                        (var_expr[..pos].to_string(), Some(var_expr[pos + 2..].to_string()))
                    } else {
                        (var_expr, None)
                    };
                    ...
                    let value = match std::env::var(&var_name) {
                        ...
                        Err(_) => {
                            if let Some(default) = default_value {
                                ...
                                return Err(format!("Environment variable {} not found and no default provided", var_name));
                            }
                        }
                    };
    ...
    pub fn find_tenant_by_request(&self, request_info: &RequestInfo) -> Option<(&TenantConfig, ResolvedUrl)> {
...
"#;
        ...
        assert_eq!(config.tenants[0].auth.token, Some("secret_token_123".to_string()));
...
"#;
        ...
        let result = AppConfig::load_from_file(temp_file);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("must contain at least one tenant"));
