...
pub async fn initialize_tenant_schemas(config: &AppConfig) -> AppResult<()> {
    ...
    if config.backend.backend_type != "database" {
        return Err(crate::error::AppError::Configuration(
            format!("Unsupported backend type: {}", config.backend.backend_type)
        ));
    }
    
    let database_config = config.backend.database.as_ref()
    ...
    let backend_config = DatabaseBackendConfig {
        database_type: match database_config.db_type.as_str() {
            ...
            _ => return Err(crate::error::AppError::Configuration(
                format!("Unsupported database type: {}", database_config.db_type)
            )),
        },
