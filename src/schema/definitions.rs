...
pub const SCIM_API_MESSAGES_LIST_RESPONSE: &str = "urn:ietf:params:scim:api:messages:2.0:ListResponse";
...
pub fn find_attribute<'a>(schema: &'a SchemaDefinition, attr_path: &str) -> Option<&'a AttributeDefinition> {
    ...
    for (i, part) in parts.iter().enumerate() {
        if let Some(attr) = current_attrs.iter().find(|a| a.name.eq_ignore_ascii_case(part)) {
            result = Some(attr);
...
mod tests {
    ...
    fn test_case_insensitive_attributes() {
        // User attributes
        assert!(is_case_insensitive_attribute("userName", ResourceType::User));
        assert!(is_case_insensitive_attribute("emails.value", ResourceType::User));
        assert!(!is_case_insensitive_attribute("id", ResourceType::User));
        assert!(!is_case_insensitive_attribute("externalId", ResourceType::User));
        
        // Group attributes
        assert!(is_case_insensitive_attribute("displayName", ResourceType::Group));
    ...
    fn test_multi_valued_attributes() {
        ...
        assert!(!is_multi_valued_attribute("displayName", ResourceType::Group));
    }
    ...
    fn test_case_exact_compatibility() {
        ...
        assert!(!is_case_exact_field_for_resource("displayName", ResourceType::User));
        
        assert!(is_multi_valued_attribute("emails", ResourceType::User));
        assert!(is_multi_valued_attribute("phoneNumbers", ResourceType::User));
        assert!(!is_multi_valued_attribute("userName", ResourceType::User));
    }
}