...
fn test_nested_schema_qualified_paths() {
    ...
    assert!(result.is_ok(), "Should successfully replace nested schema qualified attribute");
    
    let manager_value = user["urn:ietf:params:scim:schemas:extension:enterprise:2.0:User"]["manager"]["value"].as_str();
...
fn test_complex_multi_valued_attribute_filters() {
    ...
    assert!(result.is_ok(), "Should successfully replace filtered attribute");
...
fn test_case_sensitive_attribute_names() {
    ...
    assert!(result.is_ok(), "Should successfully replace case sensitive attribute");
...
fn test_deep_nested_path_with_arrays() {
    ...
    assert!(result.is_ok(), "Should successfully replace deep nested attribute");
    
    let addresses = user["addresses"].as_array().unwrap();
    assert_eq!(addresses[1]["streetAddress"].as_str(), Some("789 New Work Blvd"));
...
fn test_add_to_non_existent_array() {
    ...
    println!("User after operation: {}", serde_json::to_string_pretty(&user).unwrap());
}
...
fn test_special_characters_in_filter_values() {
    ...
    assert!(result.is_ok(), "Should handle special characters in filter values");
...
fn test_performance_with_large_arrays() {
    ...
    let target_group = groups.iter().find(|g| g["display"].as_str() == Some("Target Group"));
