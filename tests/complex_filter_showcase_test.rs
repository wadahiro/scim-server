...
fn showcase_complex_filters() {
    ...
    let simple_path = ScimPath::parse("emails[type eq \"work\"]").expect("Should parse simple filter");
    let mut user_copy = user.clone();
    simple_path.apply_operation(&mut user_copy, "remove", &json!(null))
        .expect("Should remove work emails");
    
    let remaining_emails = user_copy["emails"].as_array().unwrap();
    println!("   ✅ Removed work emails, {} emails remaining", remaining_emails.len());
    ...
    let mut user_copy = user.clone();
    and_path.apply_operation(&mut user_copy, "replace", &json!({
    ...
    let primary_work = emails.iter().find(|e| e["primary"] == true && e["type"] == "work").unwrap();
    println!("   ✅ Updated primary work email to: {}", primary_work["value"]);
    ...
    println!("   ✅ Removed {} phone numbers (work or mobile)", original_count - remaining_count);
    ...
    let precedence_path = ScimPath::parse("emails[type eq \"work\" and primary eq true or type eq \"home\"]")
    ...
    let contains_path = ScimPath::parse("emails[value co \"@company\"]")
        .expect("Should parse contains filter");
    let mut user_copy = user.clone();
    contains_path.apply_operation(&mut user_copy, "replace", &json!({
    ...
    let emails = user_copy["emails"].as_array().unwrap();
    let company_emails: Vec<_> = emails.iter()
        .filter(|e| e["value"].as_str().unwrap_or("").contains("@company"))
        .collect();
    println!("   ✅ Updated {} emails containing '@company'", company_emails.len());
