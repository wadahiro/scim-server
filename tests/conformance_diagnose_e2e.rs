...
/// in this server (`crates/scim-conformance/src/findings.rs`'s
/// `classify_regression`, adapted here to work off `Finding::key` instead
...
fn classify_regression(key: &str) -> Option<&'static str> {
    let (resource_attr, rest) = key.split_once('/')?;
    let (characteristic, method) = rest.split_once('/')?;
    let (resource, attr) = resource_attr.split_once('.')?;

    // patch-readonly-not-rejected: PATCH targeting a readOnly attribute is
    // silently ignored instead of rejected -- takes precedence over
    // readonly-extension-subattr-echo for PATCH cells that also happen to
    // echo the forged value.
    if characteristic == "mutability_readOnly" && method == "PATCH" {
        return Some("patch-readonly-not-rejected");
    }
    match (resource, characteristic, attr, method) {
        ("EnterpriseUser", "mutability_readOnly", "manager.$ref", "POST" | "PUT") => {
            Some("readonly-extension-subattr-echo")
        }
        ("EnterpriseUser", "mutability_readOnly", "manager.displayName", "POST" | "PUT") => {
            Some("readonly-extension-subattr-echo")
        }
        ("Group", "required", "displayName", "PUT-omit") => Some("required-on-put"),
        ("Group", "type_wrong", "externalId", "POST" | "PUT") => Some("type-validation-bypass"),
        ("Group", "type_wrong", a, "POST" | "PUT")
            if a == "members" || a.starts_with("members.") =>
        {
            Some("type-validation-bypass")
        }
        ("User", "type_wrong", a, "POST" | "PUT") if a.starts_with("addresses.") => {
            Some("type-validation-bypass")
        }
...
/// PR #72 fixed all five regressions this file's `classify_regression`
/// names, so against this rebase the report should have no FAILs at all.
/// It used to be reported here with a "genuine, narrow residual" on
/// `Group.members.display`: PATCH against that readOnly sub-attribute of
/// the top-level multi-valued `members` complex appeared to be silently
/// ignored instead of rejected. That was a false positive in the *probe*,
/// not a server gap -- the coarse, container-level `path: "members"`
/// request the old probe sent for this cell shape (no per-element filter
/// target during a forge) replaced the whole (ReadWrite) `members` array
/// with a structurally invalid element, which is not a probe of
/// `display`'s mutability at all. PR #73 fixed the probe to target the
...
async fn fail_set_is_empty() {
    ...
    assert_eq!(
        fails.len(),
        0,
        "expected 0 FAILs (PR #72 fixed every server-side non-conformance this file's \
         classify_regression covers, and the apparent \
         Group.members.display/mutability_readOnly/PATCH residual was a false positive in the \
         probe itself, since corrected in PR #73 -- see this test's doc comment); got {}:\n{}",
        fails.len(),
        report_lines.join("\n")
    );
}
