...
use common::{TestDatabaseType, TestDatabase};
...
async fn user_crud_test(db_type: TestDatabaseType) {
    ...
    let (app, _test_db) = common::setup_test_app_with_db(tenant_config, db_type).await.unwrap();
    ...
    let user_data = common::create_test_user_json(
        ...
        &format!("{}User", db_prefix.chars().next().unwrap().to_uppercase().collect::<String>() + &db_prefix[1..]),
        "Test"
    );
    ...
    assert_eq!(updated_user["userName"], format!("{}-user-updated", db_prefix));
...
async fn group_crud_test(db_type: TestDatabaseType) {
    ...
    assert_eq!(created_group["displayName"], format!("{} Test Group", db_prefix));
    ...
    assert_eq!(retrieved_group["displayName"], format!("{} Test Group", db_prefix));
...
async fn group_membership_test(db_type: TestDatabaseType) {
    ...
    assert_eq!(user_with_groups["groups"][0]["display"], format!("{} Membership Group", db_prefix.chars().next().unwrap().to_uppercase().collect::<String>() + &db_prefix[1..]));
}
...
async fn group_to_group_membership_test(db_type: TestDatabaseType) {
    ...
    assert_eq!(patched_parent["members"][0]["display"], format!("{} Child Group", db_prefix));
...
async fn group_patch_operations_test(db_type: TestDatabaseType) {
    ...
    assert_eq!(patched_group["displayName"], format!("Patched {} Group Name", db_prefix.chars().next().unwrap().to_uppercase().collect::<String>() + &db_prefix[1..]));
...
async fn enhanced_filter_search_test(db_type: TestDatabaseType) {
    ...
    for (username, nickname, external_id, title, user_type) in test_users {
        ...
        if let Some(id) = created_user["id"].as_str() {
            ...
            println!("   ✅ ユーザー '{}-{}' を作成 (ID: {})", db_prefix, username, id);
        }
    }
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20eq%20%22{}-alice%22", db_prefix))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    assert_eq!(search_result["Resources"][0]["userName"].as_str().unwrap(), format!("{}-alice", db_prefix));
    println!("      ✅ userName '{}-alice' での検索成功", db_prefix);

    // Test 2: nickName での検索（新機能）
    println!("\n   Test 2: nickName での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=nickName%20eq%20%22Bobby%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let search_result: Value = response.json();
    assert_eq!(search_result["totalResults"].as_i64().unwrap(), 1);
    assert_eq!(search_result["Resources"][0]["userName"].as_str().unwrap(), format!("{}-bob", db_prefix));
    println!("      ✅ nickName 'Bobby' での検索成功");

    // Test 3: externalId での検索（case-exact）
    println!("\n   Test 3: externalId での検索（case-exact）");
    let response = server
        .get(&format!("/scim/v2/Users?filter=externalId%20eq%20%22EXT-003%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let search_result: Value = response.json();
    assert_eq!(search_result["totalResults"].as_i64().unwrap(), 1);
    assert_eq!(search_result["Resources"][0]["userName"].as_str().unwrap(), format!("{}-charlie", db_prefix));
    println!("      ✅ externalId 'EXT-003' での検索成功");

    // Test 4: title での検索
    println!("\n   Test 4: title での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=title%20eq%20%22Product%20Manager%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 5: userType での検索（複数結果）");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userType%20eq%20%22Employee%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ userType 'Employee' での検索成功（{}件）", search_result["totalResults"]);

    // Test 6: emails での検索（複合フィルター）
    println!("\n   Test 6: emails での検索（複合フィルター）");
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails.value%20eq%20%22diana%40company.com%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let search_result: Value = response.json();
    assert_eq!(search_result["totalResults"].as_i64().unwrap(), 1);
    assert_eq!(search_result["Resources"][0]["userName"].as_str().unwrap(), format!("{}-diana", db_prefix));
    println!("      ✅ emails[value eq 'diana@company.com'] での検索成功");

    // Test 7: name.formatted での検索（ネストしたname属性）
    println!("\n   Test 7: name.formatted での検索（ネストしたname属性）");
    let response = server
        .get(&format!("/scim/v2/Users?filter=name.formatted%20eq%20%22Alice%20Test%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 8: name.givenName での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=name.givenName%20eq%20%22Bob%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 9: name.familyName での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=name.familyName%20eq%20%22Test%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ name.familyName 'Test' での検索成功（{}件）", search_result["totalResults"]);

    // Test 10: displayName での検索（case-insensitive）
    println!("\n   Test 10: displayName での検索（case-insensitive）");
    let response = server
        .get(&format!("/scim/v2/Users?filter=displayName%20eq%20%22ali%20%28software%20engineer%29%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails.type%20eq%20%22work%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    assert_eq!(search_result["totalResults"].as_i64().unwrap(), 5);
    println!("      ✅ emails.type 'work' での検索成功（{}件）", search_result["totalResults"]);

    // Test 12: 存在しない値での検索
    println!("\n   Test 12: 存在しない値での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20eq%20%22nonexistent%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
...
async fn nested_attributes_filter_test(db_type: TestDatabaseType) {
    ...
    let created_user: Value = response.json();
    let user_id = created_user["id"].as_str().unwrap();
    println!("   ✅ 複雑なユーザー '{}-complex' を作成 (ID: {})", db_prefix, user_id);
    ...
    println!("\n   Test 1: name.middleName での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=name.middleName%20eq%20%22Michael%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let search_result: Value = response.json();
    assert_eq!(search_result["totalResults"].as_i64().unwrap(), 1);
    assert_eq!(search_result["Resources"][0]["userName"].as_str().unwrap(), format!("{}-complex", db_prefix));
    println!("      ✅ name.middleName 'Michael' での検索成功");

    // Test 2: name.honorificPrefix での検索
    println!("\n   Test 2: name.honorificPrefix での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=name.honorificPrefix%20eq%20%22Dr.%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 3: name.honorificSuffix での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=name.honorificSuffix%20eq%20%22Jr.%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 4: profileUrl での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=profileUrl%20eq%20%22https://example.com/profile/john%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 5: phoneNumbers.value での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=phoneNumbers.value%20eq%20%22%2B1-555-123-4567%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 6: phoneNumbers.type での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=phoneNumbers.type%20eq%20%22mobile%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.locality%20eq%20%22New%20York%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 8: addresses.region での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.region%20eq%20%22NY%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 9: addresses.postalCode での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.postalCode%20eq%20%2210001%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 10: addresses.country での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.country%20eq%20%22USA%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 11: preferredLanguage での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=preferredLanguage%20eq%20%22en-US%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 12: timezone での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=timezone%20eq%20%22America/New_York%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails.type%20eq%20%22other%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
...
async fn multi_value_attributes_filter_test(db_type: TestDatabaseType) {
    ...
    let created_user: Value = response.json();
    let user_id = created_user["id"].as_str().unwrap();
    println!("   ✅ マルチバリューユーザー '{}-multivalue' を作成 (ID: {})", db_prefix, user_id);
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails%5Bvalue%20eq%20%22alice.work%40company.com%22%5D"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let search_result: Value = response.json();
    assert_eq!(search_result["totalResults"].as_i64().unwrap(), 1);
    assert_eq!(search_result["Resources"][0]["userName"].as_str().unwrap(), format!("{}-multivalue", db_prefix));
    println!("      ✅ 1つ目のemail 'alice.work@company.com' での検索成功");

    // Test 2: 複数emailsの中から2つ目をヒット
    println!("\n   Test 2: emails - 2つ目の値での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails%5Bvalue%20eq%20%22alice.personal%40gmail.com%22%5D"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails%5Bvalue%20eq%20%22a.smith%40university.edu%22%5D"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=phoneNumbers.value%20eq%20%22%2B1-555-300-3003%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=phoneNumbers.type%20eq%20%22pager%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.locality%20eq%20%22San%20Francisco%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.locality%20eq%20%22Berkeley%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.locality%20eq%20%22Miami%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.region%20eq%20%22CA%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.region%20eq%20%22FL%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=addresses.postalCode%20eq%20%2294704%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails%5Bvalue%20eq%20%22nonexistent%40email.com%22%5D"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
...
async fn enhanced_group_filter_search_test(db_type: TestDatabaseType) {
    ...
    println!("      🔍 検索フィルタ: displayName eq \"{}\"", search_filter.replace("%20", " "));
    let response = server
        .get(&format!("/scim/v2/Groups?filter=displayName%20eq%20%22{}%22", search_filter))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;

    response.assert_status(StatusCode::OK);
    let search_result: Value = response.json();
    println!("      📊 検索結果: totalResults={}", search_result["totalResults"].as_i64().unwrap());
    ...
    println!("\n   Test 2: externalId での検索（case-exact）");
    let response = server
        .get(&format!("/scim/v2/Groups?filter=externalId%20eq%20%22GRP-002%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 3: 存在しないグループでの検索");
    let response = server
        .get(&format!("/scim/v2/Groups?filter=displayName%20eq%20%22NonExistent%20Group%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
...
async fn case_sensitivity_filtering_test(db_type: TestDatabaseType) {
    ...
    println!("\n   Test 1: externalId (case-exact) - 正確なケース");
    let response = server
        .get(&format!("/scim/v2/Users?filter=externalId%20eq%20%22EXT-CASE-001%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Test 3: nickName (case-insensitive) - 大文字小文字無視");
    let response = server
        .get(&format!("/scim/v2/Users?filter=nickName%20eq%20%22testnick%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
...
async fn complex_query_patterns_test(db_type: TestDatabaseType) {
    ...
    let complex_users = vec![
        ...
        ("alice.smith", "Senior Software Engineer", "Employee", "Engineering", "ENG001", true),
        ("bob.jones", "Product Manager", "Employee", "Product", "PROD001", true),
        ("charlie.brown", "Junior Developer", "Contractor", "Engineering", "ENG001", true),
        ("diana.prince", "Director of Engineering", "Employee", "Engineering", "ENG001", true),
        ("eve.adams", "UX Designer", "Employee", "Design", "DES001", false),
        ("frank.miller", "DevOps Engineer", "Employee", "Operations", "OPS001", true),
        ("grace.hopper", "Senior Data Scientist", "Employee", "Data", "DATA001", true),
        ("henry.ford", "Quality Assurance Engineer", "Contractor", "Engineering", "ENG001", true),
        ("iris.watson", "Technical Writer", "Employee", "Documentation", "DOC001", true),
        ("jack.black", "Sales Engineer", "Employee", "Sales", "SALES001", false),
    ];
    ...
    let response = server
        .get(&format!("/scim/v2/Users?filter=department%20eq%20%22Engineering%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ Engineering部門のユーザー検索成功: {}件", engineering_users);
    ...
    println!("      ✅ 'Senior'を含むタイトル検索成功: {}件", senior_users);

    // Pattern 3: プレフィックス検索 - "ENG"で始まるコストセンター
    println!("\n   Pattern 3: コストセンターのプレフィックス検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=costCenter%20sw%20%22ENG%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 'ENG'で始まるコストセンター検索成功: {}件", eng_cost_center_users);

    // Pattern 4: サフィックス検索 - "Engineer"で終わるタイトル
    println!("\n   Pattern 4: タイトルのサフィックス検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=title%20ew%20%22Engineer%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 'Engineer'で終わるタイトル検索成功: {}件", engineer_title_users);

    // Pattern 5: 否定検索 - Employeeでないユーザー
    println!("\n   Pattern 5: 否定条件での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userType%20ne%20%22Employee%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ Employeeでないユーザー検索成功: {}件", non_employee_users);
    ...
    println!("      ✅ departmentフィールド存在チェック成功: {}件", users_with_department);

    // Pattern 7: 複雑なEmailドメイン検索
    println!("\n   Pattern 7: メールアドレスドメイン検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails.value%20ew%20%22%40company.com%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 会社メールドメイン検索成功: {}件", company_email_users);

    // Pattern 8: 名前のフォーマット検索
    println!("\n   Pattern 8: フォーマット済み名前での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=name.formatted%20co%20%22Alice%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      🔍 非アクティブユーザー検索結果: {}件", inactive_users);
    if let Some(resources) = search_result["Resources"].as_array() {
        println!("      📝 見つかったユーザー:");
        for resource in resources {
    ...
    assert!(inactive_users >= 2, "Expected at least 2 inactive users (eve, jack), found {}", inactive_users); // eve, jack
    println!("      ✅ 非アクティブユーザー検索成功: {}件", inactive_users);

    // Pattern 10: 特定のメールタイプ検索
    println!("\n   Pattern 10: 特定のメールタイプでの検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails.type%20eq%20%22home%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ ホームメールタイプ検索成功: {}件", home_email_users);

    println!("\n✅ 全ての複雑なクエリパターンテストが成功！({:?})", db_type);
...
async fn advanced_filter_operators_test(db_type: TestDatabaseType) {
    ...
    for (username, age, salary, hire_date, performance_score, manager_level) in advanced_users {
        ...
        if let Some(id) = created_user["id"].as_str() {
            created_user_ids.push(id.to_string());
            println!("   ✅ ユーザー '{}-{}' を作成 (年齢: {}, 給与: {})", db_prefix, username, age, salary);
        }
    }
    ...
    response.assert_status(StatusCode::OK);
    let search_result: Value = response.json();
    println!("      🔍 Debug: Search result: {}", serde_json::to_string_pretty(&search_result).unwrap());
    ...
    println!("      ✅ 給与85000以上のユーザー検索成功: {}件", high_salary_users);
    ...
    println!("      ✅ 給与70000以下のユーザー検索成功: {}件", low_salary_users);

    // Test 5: Not Equal (ne) - マネージャーレベル1でない
    println!("\n   Test 5: ne (Not Equal) - マネージャーレベル1でない");
    let response = server
        .get(&format!("/scim/v2/Users?filter=managerLevel%20ne%20%221%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ マネージャーレベル1でないユーザー検索成功: {}件", non_level1_users);

    // Test 6: Contains (co) - メールアドレスに\"old\"を含む
    println!("\n   Test 6: co (Contains) - メールアドレスに'old'を含む");
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails.value%20co%20%22old%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 'old'を含むメールアドレス検索成功: {}件", old_email_users);

    // Test 7: Starts With (sw) - ユーザー名が\"alex\"で始まる
    println!("\n   Test 7: sw (Starts With) - ユーザー名が特定の文字で始まる");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20sw%20%22{}-a%22", db_prefix))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 特定の文字で始まるユーザー名検索成功: {}件", alex_users);
    ...
    println!("      ✅ performanceScoreフィールド存在チェック成功: {}件", performance_users);
...
async fn edge_case_filtering_test(db_type: TestDatabaseType) {
    ...
    let edge_case_users = vec![
        ...
        ("user.with-special@chars", "test+user@company.com", "José María", true, false),
        ("user_with_underscore", "user_test@domain.co.uk", "François", false, true),
        ("user with spaces", "user.spaces@company.org", "Müller", true, false),
        ("userWithCamelCase", "camel.case@example.net", "Østergård", false, false),
        ("user123numbers", "numbers123@test.com", "田中太郎", true, true),
        ("UPPERCASE_USER", "UPPER@CASE.COM", "ALLCAPS", false, false),
        ("lowercase.user", "lower@case.com", "lowercase", true, false),
        ("user-with-dashes", "dashes-user@domain.com", "O'Connor", false, true),
    ];
    ...
    for (username, email, display_name, has_empty_field, has_null_field) in edge_case_users {
        ...
        if let Some(id) = created_user["id"].as_str() {
            created_user_ids.push(id.to_string());
            println!("   ✅ エッジケースユーザー '{}-{}' を作成", db_prefix, username);
        }
    }
    ...
    println!("\n   Edge Case 1: 特殊文字を含むユーザー名での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20co%20%22special%40chars%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 特殊文字を含むユーザー名検索成功: {}件", special_char_users);

    // Edge Case 2: Unicode文字を含む表示名での検索
    println!("\n   Edge Case 2: Unicode文字を含む表示名での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=displayName%20co%20%22Jos%C3%A9%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ Unicode文字を含む表示名検索成功: {}件", unicode_users);

    // Edge Case 3: スペースを含むユーザー名での検索
    println!("\n   Edge Case 3: スペースを含むユーザー名での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20co%20%22with%20spaces%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ スペースを含むユーザー名検索成功: {}件", space_users);

    // Edge Case 4: 大文字小文字混在での検索（case-insensitive）
    println!("\n   Edge Case 4: 大文字小文字混在での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20co%20%22uppercase%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("\n   Edge Case 6: ハイフンを含むユーザー名での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20co%20%22dashes%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ ハイフンを含むユーザー名検索成功: {}件", dash_users);

    // Edge Case 7: 複雑なメールドメインでの検索
    println!("\n   Edge Case 7: 複雑なメールドメインでの検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=emails.value%20ew%20%22.co.uk%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 複雑なメールドメイン検索成功: {}件", complex_domain_users);

    // Edge Case 8: アポストロフィを含む名前での検索
    println!("\n   Edge Case 8: アポストロフィを含む名前での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=displayName%20co%20%22O%27Connor%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ アポストロフィを含む名前検索成功: {}件", apostrophe_users);

    // Edge Case 9: 日本語文字での検索
    println!("\n   Edge Case 9: 日本語文字での検索");
    let response = server
        .get(&format!("/scim/v2/Users?filter=displayName%20co%20%22%E7%94%B0%E4%B8%AD%22"))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ nickNameフィールド存在チェック成功: {}件", nickname_present_users);
    ...
    // 空文字列のtitleを持つユーザー
    println!("      ✅ 空文字列フィールド検索成功: {}件", empty_title_users);

    // Edge Case 12: 境界値テスト - 非常に長い文字列
    println!("\n   Edge Case 12: 非常に長い検索文字列での検索");
    let long_search_string = "a".repeat(100); // 100文字の"a"
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20co%20%22{}%22", long_search_string))
        .add_header(http::header::ACCEPT, "application/scim+json")
        .await;
    ...
    println!("      ✅ 長い検索文字列での検索成功（該当なし）: {}件", long_string_users);

    println!("\n✅ 全てのエッジケースフィルタリングテストが成功！({:?})", db_type);
...
matrix_test!(enhanced_group_filter_search, enhanced_group_filter_search_test);
...
matrix_test!(multi_value_attributes_filter, multi_value_attributes_filter_test);
