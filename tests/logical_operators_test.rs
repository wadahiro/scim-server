...
async fn test_simple_and_operator_with_columns() {
    ...
    println!("DEBUG: john.doe user data: {}", serde_json::to_string_pretty(&john_user).unwrap());
    ...
    println!("DEBUG: Active filter result count: {}", active_result["totalResults"]);
    ...
    println!("DEBUG: Username filter result count: {}", username_result["totalResults"]);
    
    // Test 1: userName contains "john" AND active eq true
    // This should match only john.doe
    let response = server
        .get(&format!("/scim/v2/Users?filter=userName%20co%20%22john%22%20and%20active%20eq%20true"))
        .add_header("accept", "application/scim+json")
        .await;
    ...
    println!("DEBUG: Combined filter result: {}", serde_json::to_string_pretty(&result).unwrap());
...
async fn test_simple_or_operator_with_columns() {
    ...
    let usernames: Vec<&str> = resources.iter()
