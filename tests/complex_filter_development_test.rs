...
fn test_special_case_filters_work() {
    ...
    let test_cases = vec![
        (
            "groups[display eq \"Marketing Team\"].value",
            "Marketing Team",
        ),
        (
            "roles[displayName eq \"System Administrator\"].active",
            "System Administrator",
        ),
        (
            "departments[name eq \"Human Resources\"].location",
            "Human Resources",
        ),
    ];
...
fn test_filter_matching_logic() {
    ...
    assert!(
        result.is_ok(),
        "Should successfully remove matching elements"
    );
    ...
    assert_eq!(
        phones[0]["type"], "work",
        "Remaining phone should be work type"
    );
}
...
fn test_complex_filter_foundation() {
    ...
    match parsed {
        ScimPath::ValuePath { filter, .. } => {
            ...
            assert!(
                !filter.matches(&non_match_obj),
                "Should not match home email"
            );
        }
...
fn test_complex_filter_and_operator() {
    ...
    let result =
        parsed.apply_operation(&mut user, "replace", &json!("new-work-primary@company.com"));
    assert!(result.is_ok(), "Complex AND filter should work");

    let emails = user["emails"].as_array().unwrap();
    // Find the email that was replaced
    let work_primary_email = emails
        .iter()
        .find(|e| e["type"] == "work" && e["primary"] == true)
        .unwrap();
    ...
    assert!(emails
        .iter()
        .any(|e| e["value"] == "work-secondary@example.com"));
}
...
fn test_complex_filter_or_operator() {
    ...
    let remaining_types: Vec<&str> = phones.iter().map(|p| p["type"].as_str().unwrap()).collect();
...
fn test_complex_filter_mixed_operators() {
    ...
    let remaining_values: Vec<&str> = emails
        .iter()
        .filter(|e| e["value"] != "replaced@company.com")
...
fn test_advanced_filter_operators() {
    ...
    let test_cases = vec![
        ...
        (
            ...
            "Should match emails containing @company",
        ),
        (
            "emails[value sw \"admin\"]",
            "Should match emails starting with admin",
        ),
        (
            "emails[value ew \".org\"]",
            "Should match emails ending with .org",
        ),
    ];
    ...
    if let ScimPath::ValuePath { filter, .. } = parsed {
        for (test_obj, should_match) in test_data {
            ...
            assert_eq!(
                ...
                "Filter matching failed for {:?}, expected {}",
                test_obj, should_match
            );
        }
    } else {
