...
async fn test_photos_and_ims() {
    ...
    assert_eq!(primary_photo["value"], "https://photos.example.com/profile.jpg");
...
async fn test_x509_certificates() {
    ...
    assert_eq!(created_user["x509Certificates"].as_array().unwrap().len(), 1);
...
async fn test_multiple_primary_constraint() {
    ...
    response.assert_status(StatusCode::BAD_REQUEST);
    let error: Value = response.json();
    assert!(error["error"].as_str().unwrap().contains("At most one element can have primary=true"));
}
